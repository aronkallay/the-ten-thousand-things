...
var audio_file_locations = ["assets/audio/26'/26' - ",
                      ...
                      "assets/audio/31'/31' - ",
                      "assets/audio/34'/34' - ",
                      "assets/audio/45' for a Speaker/45' - "];
...
speaker_sound1.src = audio_file_locations[4] + speaker_sound1_fileno.toString() + ".wav";

var speaker_sound2_fileno = 2;
speaker_sound2.src = audio_file_locations[4] + speaker_sound2_fileno.toString() + ".wav";
speaker_sound2.load();
...
function make_string_longest(){
  ...
  while(curr_time > string_elapsed_time + durations_string[string_order[i]]){
    string_elapsed_time += durations_string[string_order[i]];
    i+= 1;
  }

  string_sound1_fileno = string_order[i] + 1;
  string_sound1_orderind = i;
  ...
  if(string_sound1_fileno <= durations_string.length){
    ...
    //     curr_speaker_sound.play();
    //     console.log("string and speaker playing from make string longest");
    //     start_time_display();
    //   }, {once: true});
    // }
...
function string1_longest_ended(){
  ...
  if(string_sound1_fileno < durations_string.length){
    ...
    curr_string_sound.play();
    console.log("string 2 playing from string 1 longest ended");
    //if there's a next string sound 1
    if(string_sound1_orderind + 2 < durations_string.length){
      string_sound1_orderind += 2;
      string_sound1_fileno = string_order[string_sound1_orderind] + 1;
      new_string1_sound(string_sound1_fileno, 0, true, false, false, false);
      console.log("string 1 fileno", string_sound1_fileno);
    }
  }
...
function string2_longest_ended(){
 ...
 if(string_sound2_fileno < durations_string.length){
   curr_string_sound = string_sound1;
   curr_string_sound.play();
   console.log("string 1 playing from string 2 longest ended");

   //if there's a next sound 2
   if(string_sound2_orderind +2 < durations_string.length){
     string_sound2_orderind += 2;
     string_sound2_fileno = string_order[string_sound2_orderind] + 1;
     new_string2_sound(string_sound2_fileno, 0, true);
     console.log("string 2 fileno", string_sound2_fileno);
   }
 }
...
function new_string1_sound(fileno, seek, isLongest, isCurrent, isFirst, needsWaiting){

  string_sound1_fileno = fileno;

  string_sound1.src = audio_file_locations[0] + string_sound1_fileno.toString() + ".wav";
  string_sound1.load();
  string_sound1.muted = !check1.checked || !string_mute.checked;
  ...
  if (!isLongest){
    ...
    if (!isCurrent){
      ...
      string_sound1.addEventListener("loadedmetadata", function(){
        ...
        //   string_sound1.play();
        //   console.log("string 1 playing from loadedmetadata event isCurrent");
        // }
      }, {once: true});
...
function new_string2_sound(fileno, seek, isLongest){
  ...
  string_sound2.src = audio_file_locations[0] + string_sound2_fileno.toString() + ".wav";
  string_sound2.load();
  string_sound2.muted = !check1.checked || !string_mute.checked;
...
function make_perc_longest(){
  ...
  while(curr_time > perc_elapsed_time + durations_perc[perc_order[i]]){
    perc_elapsed_time += durations_perc[perc_order[i]];
    i+= 1;
  }

  perc_sound1_fileno = perc_order[i]+1;
  perc_sound1_orderind = i;
  ...
  if(perc_sound1_fileno <= durations_perc.length){
    ...
    //     curr_speaker_sound.play();
    //     console.log("perc and speaaker plaaying from make perc longest");
    //     start_time_display();
    //   }, {once: true});
    // }
...
function new_perc1_sound(fileno, seek, isLongest, isCurrent, isFirst, needsWaiting){
  //console.log("perc 1", fileno, seek, isLongest);
  perc_sound1_fileno = fileno;

  perc_sound1.src = audio_file_locations[1] + perc_sound1_fileno.toString() + ".wav";
  perc_sound1.load();
  perc_sound1.muted = !check2.checked || !perc_mute.checked;
  ...
  if (!isLongest){
    ...
    if (!isCurrent){
      ...
      perc_sound1.addEventListener("loadedmetadata", function(){
        ...
        //   perc_sound1.play();
        //   console.log("perc 1 playing from loadedmetadata event isCurrent");
        // }
...
function new_perc2_sound(fileno, seek, isLongest){
  ...
  perc_sound2.src = audio_file_locations[1] + perc_sound2_fileno.toString() + ".wav";
  perc_sound2.load();
  perc_sound2.muted = !check2.checked || !perc_mute.checked;
...
function make_piano1_longest(){
  ...
  while(curr_time > piano1_elapsed_time + durations_piano1[piano1_order[i]]){
    piano1_elapsed_time += durations_piano1[piano1_order[i]];
    i+= 1;
  }

  piano1_sound1_fileno = piano1_order[i]+1;
  piano1_sound1_orderind = i;
  ...
  if(piano1_sound1_fileno <= durations_piano1.length){
    ...
    //     curr_speaker_sound.play();
    //     console.log("piano1 and speaker playing from make piano1 longest");
    //     start_time_display();
    //   }, {once: true});
    // }
...
function new_piano11_sound(fileno, seek, isLongest, isCurrent, isFirst, needsWaiting){
  //console.log("piano1 1", fileno, seek, isLongest);
  piano1_sound1_fileno = fileno;

  piano1_sound1.src = audio_file_locations[2] + piano1_sound1_fileno.toString() + ".wav";
  piano1_sound1.load();
  piano1_sound1.muted = !check3.checked || !piano1_mute.checked;

  if(needsWaiting){
    piano1_sound1.addEventListener("loadedmetadata", function(){
      //console.log("piano1 loaded");
      loading_proxy.curr_loaded += 1;
    }, {once: true});
  }
  ...
  if (!isLongest){
    ...
    if (!isCurrent){
      ...
      piano1_sound1.addEventListener("loadedmetadata", function(){
        ...
        //   piano1_gain.gain.exponentialRampToValueAtTime(1, audioCtx.currentTime + 0.015);
        //   piano1_sound1.play();
        //   console.log("piano1 1 playing from loadedmetadata event isCurrent");
        // }
...
function new_piano12_sound(fileno, seek, isLongest){
  ...
  piano1_sound2.src = audio_file_locations[2] + piano1_sound2_fileno.toString() + ".wav";
  piano1_sound2.load();
  piano1_sound2.muted = !check3.checked || !piano1_mute.checked;
...
function make_piano2_longest(){
  ...
  while(curr_time > piano2_elapsed_time + durations_piano2[piano2_order[i]]){
    piano2_elapsed_time += durations_piano2[piano2_order[i]];
    i+= 1;
  }

  piano2_sound1_fileno = piano2_order[i]+1;
  piano2_sound1_orderind = i;
  ...
  if(piano2_sound1_fileno <= durations_piano2.length){
    ...
    //     curr_speaker_sound.play();
    //     console.log("piaano2 and speaker playing from make piano2 longest");
    //     start_time_display();
    //   }, {once: true});
    // }
...
function new_piano21_sound(fileno, seek, isLongest, isCurrent, isFirst, needsWaiting){
  //console.log("piano2 1", fileno, seek, isLongest);
  piano2_sound1_fileno = fileno;

  piano2_sound1.src = audio_file_locations[3] + piano2_sound1_fileno.toString() + ".wav";
  piano2_sound1.load();
  piano2_sound1.muted = !check4.checked || !piano2_mute.checked;

  if(needsWaiting){
    piano2_sound1.addEventListener("loadedmetadata", function(){
      //console.log("piano2 loaded");
      loading_proxy.curr_loaded += 1;
    }, {once: true});
  }
  ...
  if (!isLongest){
    ...
    if (!isCurrent){
      ...
      piano2_sound1.addEventListener("loadedmetadata", function(){
        ...
        //   piano2_gain.gain.exponentialRampToValueAtTime(1, audioCtx.currentTime + 0.015);
        //   piano2_sound1.play();
        //   console.log('piano2 1 playing from loadedmetdata event isCurrent');
        //
        // }
...
function new_piano22_sound(fileno, seek, isLongest){
  ...
  piano2_sound2.src = audio_file_locations[3] + piano2_sound2_fileno.toString() + ".wav";
  piano2_sound2.load();
  piano2_sound2.muted = !check4.checked || !piano2_mute.checked;
...
function piano21_longest_ended(){
  ...
  if(piano2_sound1_fileno < durations_piano2.length){
    curr_piano2_sound = piano2_sound2;
    curr_piano2_sound.play();
    //console.log("piano2 2 playing from piano21 longest ended");
    //if there's a next piano2 sound 1
    if(piano2_sound1_orderind + 2 < durations_piano2.length){
...
function piano22_longest_ended(){
 ...
 if(piano2_sound2_fileno < durations_piano2.length){
   curr_piano2_sound = piano2_sound1;
   curr_piano2_sound.play();
   //console.log("piano2 1 playing from piaano22 longest ended");
   //if there's a next sound 2
   if(piano2_sound2_orderind +2 < durations_piano2.length){
...
//***********************************************************************************
// some consolidating & misc variables **********************************************
...
//for synchronizing audio file playing
var load_waiting = {
  num_to_wait: 4,
  curr_loaded: 0
}
var loading_proxy = new Proxy(load_waiting, {
  ...
  set: function(target, property, value){
    ...
    if(target.curr_loaded == target.num_to_wait && target.num_to_wait != 0){
      console.log("ALL LOADED, ready to play");
      //console.log(string_on, perc_on, piano1_on, piano2_on);
      //console.log(longest);
      //console.log(playing);
      if(playing){
        playing = false; //tricks playing into starting again lol
        play();
      }
    }
...
//wiper function
wiper.addEventListener('change', function(){
  wiper_clicked();
  this.blur();
});
...
function wiper_clicked(){
  ...
  setTimeout(function(){
    ...
    //make longest files, if speaker is not longest
    if(longest != 5 && longest != 0){
      make_longest_funcs[longest-1]();
    }

    //find elapsed time from speaker durations
    var i = 0;
    elapsed_time = 0;
...
function string_event(){
  ...
  string_gain.gain.exponentialRampToValueAtTime(1, audioCtx.currentTime + 0.015);
  curr_string_sound.play();
  //console.log("string playing from string event");
  if (curr_string_sound == string_sound1){
    ...
    if(next_events[0] < durations_string.length){
      ...
      new_string1_sound(string_sound1_fileno, 0, false, false, false, false);
    }
  }
...
function perc_event(){
  ...
  perc_gain.gain.exponentialRampToValueAtTime(1, audioCtx.currentTime + 0.015);
  curr_perc_sound.play();
  //console.log("perc playing from perc event");
  if (curr_perc_sound == perc_sound1){
    ...
    if(next_events[1] < durations_perc.length){
      ...
      new_perc1_sound(perc_sound1_fileno, 0, false, false, false, false);
    }
  }
}
function piano1_event(){
  ...
  piano1_gain.gain.exponentialRampToValueAtTime(1, audioCtx.currentTime + 0.015);
  curr_piano1_sound.play();
  // console.log("piano1 playing from piano1 event");
  ...
  if (curr_piano1_sound == piano1_sound1){
    ...
    if(next_events[2] < durations_piano1.length){
      ...
      new_piano11_sound(piano1_sound1_fileno, 0, false, false, false, false);
    }
  }
}
function piano2_event(){
  ...
  piano2_gain.gain.exponentialRampToValueAtTime(1, audioCtx.currentTime + 0.015);
  curr_piano2_sound.play();
  //console.log("piano2 playing from piano2 event");
  if (curr_piano2_sound == piano2_sound1){
    ...
    if(next_events[3] < durations_piano2.length){
      ...
      new_piano21_sound(piano2_sound1_fileno, 0, false, false, false, false);
    }
  }
}
...
//functions for animating audio displays and track loading
var animate_tracks = false;
var track_animation_counter = 0;
function animation_loop(){

  //if playing, monitor tracks
  if(playing){
    if(monitor_speaker){
  ...
  //if tracks need loading, load tracks
  if(animate_tracks){
...
//functions dealing with sound stuff for loading/randomizing
...
function draw_colored_tracks(){
  ...
  //for the longest track, draw unbroken
  if (longest != 5 && longest != 0 ){
  ...
  if(!longest_changed && longest != 5 && longest!= 0){
    ...
    if(longest_shuffled){
      ctx = track_ctxs[longest-1]; //canvas context
      durs = all_durations[longest-1]; //durations of each segment
      num_segs = all_num_segs[longest-1]; //number of segments
      seg_locs = all_seg_locs[longest-1]; //segment locations on timeline
      order = all_orders[longest-1];
      xpos = 0;

      for (var j = 0; j < num_segs; j++){
        width = Math.round(durs[order[j]] / (lengths[longest] * 60.) * 1198) + 3;
        ctx.strokeStyle = "black";
        ctx.lineWidth = 2;
        ctx.strokeRect(xpos, 0, width, 100);
        xpos += width;
      }
    }
...
function randomize_track_segments(){
  ...
  for (var i = 0; i < longest-1; i++){
    ...
    num_segs = all_num_segs[i]; //number of segments
    seg_locs = all_seg_locs[i]; //segment locations on timeline
    order = all_orders[i]; //order of segments (index of durations)
...
function get_events(){
  ...
  for (var i = 0; i < all_seg_locs.length; i++){
    ...
    if (next_events[i] != 0){
      ...
      //if we're in the middle of a current event, add to current events
      ...
      //if the first event starts at 0, which it shouldn't, but it uh...happens anyways sometimes
      if (seg_locs[0] == 0){
...
function load_sound_files(){
  ...
  // else if (longest == 1){
  //   console.log("string test waiting for 1 sound file to load");
  //   loading_proxy.curr_loaded = 0 ;
  ...
  setTimeout(function(){
    ...
    if (longest_changed){
      ...
      if (longest != 5 && longest != 0){
        ...
        //   speaker_gain.gain.exponentialRampToValueAtTime(1, audioCtx.currentTime + 0.015);
        //   curr_speaker_sound.play();
        //   console.log("speaker playing from longest changed == 5 load sound files");
        //   start_time_display();
        // }
...
//find and load non-longest sound files
function load_non_longest_files(){
  ...
  //seeing whether we're in a current event or not
  ...
  for (var i = 0; i < longest - 1; i++){
    ...
    if (next_events[i] < all_durations[i].length){
      if (current_events[i] == null){
        ...
        sound_funcs[0](filenos[0], 0, false, false, true, true);
      }
      else{
...
//functions dealing with non-sound stuff for loading/re-randomizing
